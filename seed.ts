...
const URLS_SUPABASE = [
  ...
  "https://dzjjioioqbwiouusrkrb.supabase.co/storage/v1/object/public/fotos-clinica/WhatsApp%20Image%202026-06-01%20at%2022.47.48.jpeg"  // 19: Dr. Fernando Filho (Sem o (1) conforme deduzido da última URL do folder)
];

async function main() {
  console.log('Iniciando semeadura do banco de dados com os dados CORRETOS Luna & Mendes...');

  // 1. Limpeza segura de dados anteriores para garantir que não haja duplicatas
  ...
  // 3. Cadastrar os 20 profissionais perfeitamente alinhados com suas imagens/encartes
  const medicosReais = [
    {
  ...
  // 4. Criação de horários de atendimento reais para cada um dos novos profissionais
  const dataRef = new Date();
  dataRef.setHours(9, 0, 0, 0); // Horário de início às 09:00

  for (const med of medicosCriados) {
    // Definimos intervalos de atendimento para amanhã (+1 dia) às 09:00 e 14:00
